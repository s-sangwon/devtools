...
SELECT SUM(SALARY) 급여합계,
            SUM(DISTINCT SALARY) "중복제외 급여합계"
FROM EMPLOYEE
WHERE DEPT_ID = '50' OR DEPT_ID IS NULL;

SELECT DEPT_ID, SALARY 급여합계
FROM EMPLOYEE
WHERE DEPT_ID = '50' OR DEPT_ID IS NULL;

-- WHERE 절에서 비교값에 그룹함수 사용할 수 없음
-- WHERE 절에는 단일행함수만 사용할 수 있음
-- 예 : 전 직원의 급여 평균 보다 급여를 많이 받는 직원 조회
...
-- CHAR 2천바이트 VARCHAR2 4천바이트 LONG 2기가
...
-- COUNT(*)  | COUNT(컬럼명) | COUNT(DISTINCT 컬러명)
-- * : 전체 행 갯수 반환 (NULL 포함)
...
SELECT COUNT(*), -- 조회된 전체 행 갯수 : 8행
            ...
            COUNT(DISTINCT DEPT_ID) -- 중복 제외한 행 갯수 : 1행
FROM EMPLOYEE
WHERE DEPT_ID = '50' OR DEPT_ID IS NULL;
...
-- LTRIM('문자리터럴' | 문자가 기록됨 컬럼명, '제거할 문자들')
...
-- 암쪽 함수가 반환한 값을 바깥 함수가 사용한다는 의미임
...
-- 직원 테이블에서 사번, 이름, 주민번호 저회 

SELECT EMP_ID 사번, EMP_NAME 이름, RPAD(SUBSTR(EMP_NO,1,7), 14, '*') 주민번호
FROM EMPLOYEE;
...
-- 반올림할 자릿수가 음수이면 소숫점 앞(정수부) 올림함 자리를 의미
SELECT 123.456,
            ROUND(123.456), -- 123
            ROUND(123.456, 0), -- 123
...
SELECT EMP_ID, EMP_NAME,
            SALARY, BONUS_PCT, 
            ROUND(((SALARY + SALARY * NVL(BONUS_PCT, 0)) * 12),-4) * 12 "1년급여"
FROM EMPLOYEE;

-- TRUNC(숫자 | 숫자가 기록된 컬럼명 | 계산식, 자를 자릿수)
-- 지정한 자리 내림연산, 절삭함수, 반올림 없음
SELECT 145.678,
            TRUNC(145.678), -- 145
            TRUNC(145.678, 0), -- 145
...
--오라클 에서는환경설정, 객체 관련 정모들을 모두 저장 관리하고 있음
...
-- 직원들의 이름, 입사일, 현재까지의 근무 일수, 근무 개월수, 근무년수 조회
...
-- PDF없는거 NEXT_DAY('날짜' | 날짜가 기록된 컬럼명, '요일이름')
...
-- 직원 정보에서 이름, 입사일, 입사한 첫달의 근무일수 조회
-- 주말 포함 일수
SELECT EMP_NAME 이름, HIRE_DATE 입사일, LAST_DAY(HIRE_DATE) - HIRE_DATE 첫달근무일수
FROM EMPLOYEE;
