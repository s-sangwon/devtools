...
--한다.
SELECT department_no 학과번호, EXTRACT(YEAR FROM SYSDATE) - (MIN(SUBSTR(PROFESSOR_SSN,1,2)) + 1900) 나이
...
FROM tb_student 
LEFT JOIN tb_professor ON COACH_PROFESSOR_NO = PROFESSOR_NO
WHERE STUDENT_NAME = '서가람';  -- 서가람 학생의 지도교수 노현주 P095
...
--(선수과목이 없을 출력되지 않는다.)
SELECT *--T1.CLASS_NAME 과목, T1.preattending_class_no 선수과목
FROM TB_CLASS T1, TB_CLASS T2, TB_CLASS T3 
WHERE T1.DEPARTMENT_NO = T2.DEPARTMENT_NO
AND T1.PREATTENDING_CLASS_NO = T3.PREATTENDING_CLASS_NO
AND T1.DEPARTMENT_NO = (SELECT DEPARTMENT_NO
                                          FROM TB_DEPARTMENT
...
SELECT *--T1.CLASS_NAME 과목,  T1.CLASS_NAME 선수과목
FROM TB_CLASS T1, TB_CLASS T2
WHERE T1.PREATTENDING_CLASS_NO = T2.PREATTENDING_CLASS_NO
AND T1.DEPARTMENT_NO = (SELECT DEPARTMENT_NO
                                          FROM TB_DEPARTMENT
                                          WHERE DEPARTMENT_NAME = '행정학과' );
...
AND T1.DEPARTMENT_NO = (SELECT DEPARTMENT_NO
                                          FROM TB_DEPARTMENT
                                          WHERE DEPARTMENT_NAME = '행정학과' )
AND T1.PREATTENDING_CLASS_NO IS NOT NULL
--GROUP BY T1.CLASS_NAME;